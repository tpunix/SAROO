...
#define SECTOR_BUF_SIZE (28*1024)
u64 local_buf[SECTOR_BUF_SIZE/8];

__task void disk_task(void)
{
	// 内部扇区缓存, 32K大小, 大概13个2352的扇区. 位于SRAM中
	// FATFS层以512字节为单位读取, 每次读64个单位.
	u8 *sector_buffer = (u8*)local_buf;
...
_restart_nowait:
		...
		while(cdb.fad<cdb.play_fad_end){
			...
			if(fad>=buf_fad_start && fad<buf_fad_end){
				...
				retv = f_read(play_track->fp, sector_buffer, SECTOR_BUF_SIZE, (u32*)&nread);
				if(retv==0){
...
void sirq_task(void)
{
	...
	while(1){
		...
		if((stat&0x0007)==0){
			ST_CTRL |= 7;
			os_sem_wait(&sem_wait_irq, 0xffff);
			continue;
		}
...
void ss_sw_init(void)
{
	...
	for(i=0; i<MAX_BLOCKS; i++){
		cdb.block[i].size = -1;
		cdb.block[i].data = (u8*)(0x61a00000+i*2352);
	}

	u8 *info_buf = (u8*)0x61a00000+MAX_BLOCKS*2352; // 0x72d80
...
void saturn_config(void)
{
	...
	os_sem_init(&sem_wait_irq, 0);
	os_sem_init(&sem_wait_disc,0);

	os_tsk_create(sirq_task, 70);
	os_tsk_create(disk_task, 60);
